...
func (f *fallbackRoundTripper) NetworkObservation(req *http.Request) networkObservation {
	if req != nil && req.URL != nil {
		if IsAnthropicUpstreamURL(req.URL) {
			return resolveNetworkObservation(f.anthropic, req)
		}
		if req.URL.Scheme == "https" && strings.EqualFold(req.URL.Hostname(), "chatgpt.com") {
			return resolveNetworkObservation(f.chrome, req)
		}
	}
...
func NewUtlsHTTPClient(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, timeout time.Duration) *http.Client {
	...
	case proxyURL != "":
		...
		chromeRT = &proxyObservationRoundTripper{
			base:        chromeRT,
			observation: configuredObservation,
		}
		anthropicRT = &proxyObservationRoundTripper{
			base:        anthropicRT,
			observation: configuredObservation,
		}
