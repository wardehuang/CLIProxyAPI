...
func (h *Host) callRequestFinalizer(ctx context.Context, pluginID string, finalizer pluginapi.RequestFinalizer, req pluginapi.RequestFinalizeRequest) (out pluginapi.RequestFinalizeResponse, ok bool) {
	if h == nil || finalizer == nil || h.isPluginFused(pluginID) {
		return pluginapi.RequestFinalizeResponse{}, false
	}
	defer func() {
		if recovered := recover(); recovered != nil {
			h.fusePlugin(pluginID, "RequestFinalizer.FinalizeRequest", recovered)
			out = pluginapi.RequestFinalizeResponse{}
			ok = false
		}
	}()
	resp, errFinalize := finalizer.FinalizeRequest(ctx, req)
	if errFinalize != nil {
		log.Warnf("pluginhost: request finalizer %s failed: %v", pluginID, errFinalize)
		return pluginapi.RequestFinalizeResponse{}, false
	}
	return resp, true
}
...
func (h *Host) FinalizeRequest(ctx context.Context, req pluginapi.RequestFinalizeRequest) pluginapi.RequestFinalizeResponse {
	return h.FinalizeRequestExcept(ctx, req, "")
}
...
func (h *Host) FinalizeRequestExcept(ctx context.Context, req pluginapi.RequestFinalizeRequest, skipPluginID string) pluginapi.RequestFinalizeResponse {
	...
	for _, record := range h.Snapshot().records {
		...
		if h.isPluginFused(record.id) || finalizer == nil || record.id == skipPluginID {
			continue
		}
		nextReq := req
		...
		if resp, ok := h.callRequestFinalizer(ctx, record.id, finalizer, nextReq); ok {
...
func (h *Host) HasRequestFinalizers() bool {
	if h == nil {
		return false
	}
	for _, record := range h.Snapshot().records {
		...
		if record.plugin.Capabilities.RequestFinalizer != nil {
			return true
		}
	}
	return false
...
func mergeHeadersPreserveCase(current, updates http.Header, clear []string) http.Header {
	out := cloneHeader(current)
	if out == nil {
		out = make(http.Header)
	}
	for _, key := range clear {
		deleteHeaderCaseInsensitive(out, key)
	}
	for key, values := range updates {
		deleteHeaderCaseInsensitive(out, key)
		out[key] = append([]string(nil), values...)
