...
type pluginConfig struct {
	...
	SchemasDir     string        `json:"schemas-dir" yaml:"schemas-dir"`
	...
	// OnlyEmpty limits replacement to tools whose input_schema/parameters is empty.
	// Default true: never overwrite complete native tool schemas.
	OnlyEmpty *bool `json:"only-empty" yaml:"only-empty"`
	// InjectMissing appends registry MCP tools that are absent from request tools[].
	// Default true: fixes Cursor sessions that omit MCP tools entirely.
	InjectMissing *bool `json:"inject-missing" yaml:"inject-missing"`
}
...
func defaultPluginConfig() pluginConfig {
	...
	injectMissing := true
	return pluginConfig{
		SchemasDir:    "mcp-schemas",
		LogsDir:       "logs",
		DetailLog:     detailLogModeAuto,
		OnlyEmpty:     &onlyEmpty,
		InjectMissing: &injectMissing,
	}
}
...
func configurePlugin(configYAML []byte) {
	...
	if len(bytes.TrimSpace(configYAML)) > 0 {
		...
		if errUnmarshal := yaml.Unmarshal(configYAML, &parsed); errUnmarshal == nil {
			...
			if strings.TrimSpace(parsed.SchemasDir) != "" {
				config.SchemasDir = strings.TrimSpace(parsed.SchemasDir)
			}
			...
			if parsed.OnlyEmpty != nil {
				config.OnlyEmpty = parsed.OnlyEmpty
			}
			if parsed.InjectMissing != nil {
				config.InjectMissing = parsed.InjectMissing
			}
		}
	}
	...
	logPluginInfo("", "mcp schema registry loaded", map[string]any{
		"schemas_dir":    resolveSchemasDir(),
		"tool_count":     result.ToolCount,
		"file_count":     result.FileCount,
		"error_count":    len(result.Errors),
		"only_empty":     onlyEmptyEnabled(),
		"inject_missing": injectMissingEnabled(),
		"detail_log":     string(currentPluginConfig().DetailLog),
		"loaded_tools":   result.ToolNames,
...
func onlyEmptyEnabled() bool {
	config := currentPluginConfig()
	if config.OnlyEmpty == nil {
		return true
	}
	return *config.OnlyEmpty
}

func injectMissingEnabled() bool {
	config := currentPluginConfig()
	if config.InjectMissing == nil {
		return true
	}
	return *config.InjectMissing
}

func resolveSchemasDir() string {
	config := currentPluginConfig()
	schemasDir := strings.TrimSpace(config.SchemasDir)
	if schemasDir == "" {
		schemasDir = "mcp-schemas"
	}
	...
	return filepath.Join(workingDirectory, schemasDir)
}
