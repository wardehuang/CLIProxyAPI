...
type requestMetadataEnrichRequest struct {
	pluginapi.RequestMetadataEnrichRequest
	HostCallbackID string `json:"host_callback_id,omitempty"`
}

type routeRewriteRequest struct {
	pluginapi.RouteRewriteRequest
	HostCallbackID string `json:"host_callback_id,omitempty"`
}
...
func handleMethod(ctx context.Context, method string, request []byte) ([]byte, error) {
	...
	case pluginabi.MethodRequestMetadataEnrich:
		...
		if errUnmarshal := json.Unmarshal(request, &req); errUnmarshal != nil {
			return nil, fmt.Errorf("decode request metadata enrich request: %w", errUnmarshal)
		}
		return okEnvelope(enrichRequestMetadata(ctx, req.RequestMetadataEnrichRequest, req.HostCallbackID))
	case pluginabi.MethodRouteRewrite:
		var req routeRewriteRequest
		if errUnmarshal := json.Unmarshal(request, &req); errUnmarshal != nil {
			return nil, fmt.Errorf("decode route rewrite request: %w", errUnmarshal)
		}
		return okEnvelope(rewriteCompactRoute(ctx, req.RouteRewriteRequest, req.HostCallbackID))
	default:
