...
const (
	pluginName    = "cpa-compact-route-rewriter"
	pluginVersion = "0.1.0"
)
...
func handleMethod(ctx context.Context, method string, request []byte) ([]byte, error) {
	...
	case pluginabi.MethodRequestMetadataEnrich:
		...
		return okEnvelope(enrichRequestMetadata(ctx, req.RequestMetadataEnrichRequest))
	case pluginabi.MethodRouteRewrite:
		...
		return okEnvelope(rewriteCompactRoute(ctx, req.RouteRewriteRequest))
	default:
