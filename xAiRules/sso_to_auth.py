...
def inspect_sso_account_state(
    ...
    try:
        ...
        if parsed["denied"] or parsed["bot_flag_source"] in (1, 2):
            log(
                "  ❌ 注册风控状态(被标记): "
...
# --- Grok2API 管理端 API 直传 --------------------------------------------
# grok2api 不读 auth 目录：账号存 SQLite（/app/data/backend.db），只能通过
# 管理端 Web 或 API 导入。这里实现「登录拿 Bearer → 平铺 Build JSON →
# POST /api/admin/v1/accounts/import」的自动对接。
GROK2API_ADMIN_LOGIN_URL = "/api/admin/v1/auth/login"
GROK2API_ADMIN_IMPORT_URL = "/api/admin/v1/accounts/import"


def token_to_grok2api_build(token: dict, email: str = "") -> dict:
    """token dict → grok2api 接受的平铺 Build JSON（provider=grok_build）。

    grok2api 拒绝旧 Map 嵌套格式（{"issuer::client_id": {...}}），只接受
    importedCredentialEntry 平铺字段（见 backend/internal/infra/provider/cli/import.go）。
    """
    ...
    if not email:
        email = id_payload.get("email") or payload.get("email") or ""
    user_id = payload.get("sub") or id_payload.get("sub") or ""
    team_id = payload.get("team_id") or id_payload.get("team_id") or ""
    expires_at = ""
    if "exp" in payload:
    ...
    return {
        "provider": "grok_build",
        "name": email or user_id or "Grok Build account",
        "client_id": CLIENT_ID,
        "access_token": access,
        "refresh_token": refresh,
        "id_token": id_token,
...
def upload_grok2api_build(
    ...
    """通过 grok2api 管理端 API 上传一个 Build 账号（登录→import）。

    返回 grok2api 导入响应摘要；HTTP 失败抛 RuntimeError。
    base_url 示例：http://127.0.0.1:8318
    """
    base = str(base_url or "").strip().rstrip("/")
    user = str(username or "").strip()
    pwd = str(password or "").strip()
    if not base:
        raise ValueError("grok2api_admin_url 为空")
    if not user or not pwd:
        raise ValueError("grok2api 管理端账号/密码为空")

    # 1) 登录拿 Bearer accessToken
    login_url = f"{base}{GROK2API_ADMIN_LOGIN_URL}"
    proxies = {"http": proxy, "https": proxy} if proxy else None
    ...
    if login_resp.status_code >= 400:
        body = (login_resp.text or "").strip()
        if len(body) > 300:
            body = body[:300] + "..."
        raise RuntimeError(f"grok2api 登录失败 HTTP {login_resp.status_code}: {body or login_resp.reason}")
    login_payload = {}
    try:
    ...
    try:
        access_token = str(
            (login_payload.get("data") or {}).get("tokens") or {}
        ).strip() and str(((login_payload.get("data") or {}).get("tokens") or {}).get("accessToken") or "").strip()
    except Exception:
        access_token = ""
    if not access_token:
        raise RuntimeError("grok2api 登录成功但未取到 accessToken")

    # 2) 平铺 Build JSON → multipart 上传 import
    build = token_to_grok2api_build(token, email=email)
    build_json = json.dumps(build, ensure_ascii=False).encode("utf-8")
    fname = f"grok_build_{_safe_email_for_filename(email or build.get('user_id') or 'account')}.json"
    import_url = f"{base}{GROK2API_ADMIN_IMPORT_URL}"
    from curl_cffi import CurlMime
    mime = CurlMime()
    mime.addpart(name="files", content_type="application/json", filename=fname, data=build_json)
    ...
    if import_resp.status_code >= 400:
        raise RuntimeError(f"grok2api 导入失败 HTTP {import_resp.status_code}: {body or import_resp.reason}")
    return f"HTTP {import_resp.status_code} {body}"
...
def run_check_sso_state(
    ...
    *,
    ...
    """批量读取 grok.com 账号风控状态（botFlagSource / policy deny），不换 token 不入库。
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        metavar="FILE",
        default=None,
        help="与 --check-sso-state 联用：导出干净（botFlagSource=0）sso 原始行 txt，每行一条，可直接复用为 --sso 输入",
    )
    args = ap.parse_args()
