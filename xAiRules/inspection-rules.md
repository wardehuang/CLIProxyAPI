...
- 完整 SSE 的降智 TPS 分子固定为 `output_tokens + reasoning_tokens`，分母为从首个上游流 payload 到流完成的生成窗口，统一按整数毫秒截断，最小为 `1ms`。定时智商检测与实时守护使用相同公式。TPS 达到 hard 阈值时，不论是否出现 thinking delta，均判为 `suspected_degradation`，原因 `hard_tps`，等级 `hard`。TPS 达到 soft 阈值且未出现 thinking delta 时，判为 `suspected_degradation`，原因 `soft_tps_missing_thinking_delta`，等级 `soft`。TPS 低于 soft 阈值时，即使未出现 thinking delta 也正常；TPS 位于 soft 与 hard 阈值间且出现 thinking delta 也正常。
- xAI 流式请求的 usage event 在上游流完成后再发布。字段语义：`latency_ms` = 端到端请求耗时（UsageReporter 创建到 publish）；`ttft_ms` = executor 实测上游首响应字节耗时（RoundTrip 到 body 首读）；`generation_ms` = 首个下游 payload 到流结束的生成窗口。三者独立上报，互不改写。Manager【请求监控】展示耗时用 `latency_ms`、首字用 `ttft_ms`。TPS 优先 `(output_tokens + reasoning_tokens) / generation_ms`（与实时守护/定时智商检测生成窗口一致）；无 `generation_ms` 时回退 `(output_tokens + reasoning_tokens) / (latency_ms - ttft_ms)`。该 usage event 的 total latency、网络和鉴权等待不作为降智 TPS 分母。
