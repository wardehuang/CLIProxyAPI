...
- xAI 流式请求的 usage event 在上游流完成后再发布，并把同一生成窗口写为 `latency_ms`、`ttft_ms=0`；因此 Manager【请求监控】的 TPS 同样以 `(output_tokens + reasoning_tokens) / latency_ms` 计算，与实时守护完全一致。该 usage event 的 total latency、网络和鉴权等待不再参与降智 TPS。
