...
type Record struct {
	...
	Thinking bool
	// Compact marks requests generated by conversation compaction.
	Compact bool
...
// WithThinking stores whether the request explicitly enabled thinking for usage sinks.
func WithThinking(ctx context.Context, thinking bool) context.Context {
	if ctx == nil {
		ctx = context.Background()
	}
	return context.WithValue(ctx, thinkingContextKey{}, thinking)
}

// ThinkingFromContext reports whether the request explicitly enabled thinking.
func ThinkingFromContext(ctx context.Context) bool {
	if ctx == nil {
		return false
	}
	value, ok := ctx.Value(thinkingContextKey{}).(bool)
	return ok && value
}

// WithCompact marks the context as a conversation compaction request for usage sinks.
func WithCompact(ctx context.Context, compact bool) context.Context {
	...
	return context.WithValue(ctx, compactContextKey{}, true)
}

// CompactFromContext reports whether the request was marked as conversation compaction.
func CompactFromContext(ctx context.Context) bool {
	if ctx == nil {
		return false
	}
	value, ok := ctx.Value(compactContextKey{}).(bool)
	return ok && value
}
